...
const OwnerProtectedRoute = ({ children }) => {
  ...
  useEffect(() => {
    const checkUserAccess = async () => {
      ...
      try {
        ...
        console.log('Owner Protection - User access check:', {
          isAuthenticated: true,
          role: userData.role || 'driver',
          isSuperUser: userData.is_superuser || false
        });
  ...
  // Check if user has owner/garage role (removed verification status check)
  if (userRole !== 'garage_owner') {
    return <Navigate to="/not-authorized?reason=insufficient_permissions" replace />;
  }

  // User is authenticated and has garage_owner role - let the wrapper handle garage verification
  return children;
};
